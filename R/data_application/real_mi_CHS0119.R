...
work.data<-st_final[order(st_final$id),]
cal_tau<-function(j){
  y2<-vector()
  y1<-vector()
  s1<-vector()
...
cat(capture.output(print(tau), file="/Users/mengluliang/Dropbox/biostatisitcs_PSU/RA/2020Spring/BayesianJointModel/03results/output_CHS_MI_tau.txt"))

plot(tau, main="MI_CHS") 
...
result_cop=.C("weibul",b1=as.double(b1_sample), b2=as.double(b2_sample),b3=as.double(b3_sample),b4=as.double(b4_sample), b5=as.double(b5_sample), b6=as.double(b6_sample),b7=as.double(b7_sample),b8=as.double(b8_sample),
          ...
          t1=as.double(st_final$y1/365),t2=as.double(st_final$y2/365), s1=as.integer(st_final$s3),
...
result_dep=.C("weibul",b1=as.double(b1_sample), b2=as.double(b2_sample),b3=as.double(b3_sample),b4=as.double(b4_sample),
          ...
          x3=as.double(st_final$GEND01.x), x4=as.double(st_final$RACE01.x), t1=as.double(st_final$y1/365),t2=as.double(st_final$y2/365), s1=as.integer(st_final$s3),
          s2=as.integer(st_final$s2), id=as.integer(st_final$id-1), no_pat_p=as.integer(length(unique(st_final$id))), n_burn= as.integer(n_burn),
...
for(t_prime in t_s){
  ...
  for (t_final in t_f){
    ...
    for(k in 1:n_predict){
      ...
      if(nrow(main_predict_i2)==1| main_predict_i2$y1[1]<t_prime) {next}
      main_predict_p2=main_predict_i2[main_predict_i2$t3<t_prime,]
      if(length(main_predict_p2$y1)==0) {main_predict_p2$y1==0}
      s1_t2[k]= s1[k]
      y1_t2[k]= y1[k]
      s1_t[k]= s1[k]
      y1_t[k]= y1[k]
      ...
      if (is.na(y1_t2[k])|y1_t2[k]==0) {y1_t2[k]<-rexp(1,rate=1/(b1_est*main_predict_p2$x1[1]+b2_est*main_predict_p2$x2[1]+b3_est*main_predict_p2$x3[1]+b4_est*main_predict_p2$x4[1]+b01_est+phiN_est2))} 
      ...
      if (is.na(y1_t[k])|y1_t[k]==0) {y1_t[k]<-rexp(1,rate=1/(b1_est1*main_predict_p2$x1[1]+b2_est1*main_predict_p2$x2[1]+b3_est1*main_predict_p2$x3[1]+b4_est1*main_predict_p2$x4[1]+b01_est1+phiN_est))} 
...
saveRDS(result,paste0("CHS_MI_",nsim,"_0119.rds"))
...
BS<-function(result){
  ...
  #print(res)
  #sink()
  cat(capture.output(print(res), file="output_CHS_MI_0119.txt"))
  ...
  write.csv(temp, file="BS_terminal_6year_CHS_MI_0119.csv")
  ...
  colnames(temp)<-c("t","terminal_mean","terminal_mean_old")
  write.csv(temp, file="BS_terminal_9year_CHS_MI_0119.csv")
  ...
  colnames(temp)<-c("t","terminal_mean","terminal_mean_old")
  write.csv(temp, file="BS_terminal_12year_CHS_MI_0119.csv")
  
  
  png("BS_CHS_MI_0119.png",width=900, height=900)
...
out2<-function(st_final,n.sim){
  ...
  test.data$x1=test.data$mi_st
  test.data$x2=test.data$GEND01.x
  test.data$x3=test.data$RACE01.x
  test.data$x4=test.data$AVZMSYS
...
BS<-function(result,res){
  ...
  #print(res)
  #sink()
  cat(capture.output(print(res), file=paste0("output_WSFM_CHS_MI_0119.txt")))
  
  temp<-as.data.frame(cbind(bs_t[[1]],bs_t_sd[[1]][,2],bs_t_terminal[[1]][,2],
                            bs_t_terminal_sd[[1]][,2],bs_t_terminal_SF[[1]][,2],
                            bs_t_terminal_SF_sd[[1]][,2]))
  colnames(temp)<-c("t","terminal_mean","terminal_sd","terminal_mean_old","terminal_sd_old")
  write.csv(temp, file=paste0("BS_terminal_6yr_WSFM_CHS_MI0119.csv"))
  
  temp<-as.data.frame(cbind(bs_t[[2]],bs_t_sd[[2]][,2],bs_t_terminal[[2]][,2],
                            bs_t_terminal_sd[[2]][,2],bs_t_terminal_SF[[2]][,2],
                            bs_t_terminal_SF_sd[[2]][,2]))
  colnames(temp)<-c("t","terminal_mean","terminal_sd","terminal_mean_old","terminal_sd_old")
  write.csv(temp, file=paste0("BS_terminal_9yr_WSFM_CHS_MI0119.csv"))
  
  temp<-as.data.frame(cbind(bs_t[[3]],bs_t_sd[[3]][,2],bs_t_terminal[[3]][,2],
                            bs_t_terminal_sd[[3]][,2],bs_t_terminal_SF[[3]][,2],
                            bs_t_terminal_SF_sd[[3]][,2]))
  colnames(temp)<-c("t","terminal_mean","terminal_sd","terminal_mean_old","terminal_sd_old")
  write.csv(temp, file=paste0("BS_terminal_12yr_WSFM_CHS_MI0119.csv"))
  
  
  png(paste0("BS_WSFM_MI_CHS_0119.png"),width=900, height=900)

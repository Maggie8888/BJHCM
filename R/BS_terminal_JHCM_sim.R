#' ####################################################################
#' ########### This function simulates data taken from JHCM #########
#' ################and return BS for TVJHCM, JHCM and JFM #############
#' ############ input #########
...
#' @return BS in simulation when true data is from JHCM
#' @export

BS_JHCM<-function(idx,n.sim){
    ##idx=1 for the scenario with sample size n=100 and theta=1,death rate=0.6; idx=2 for the scenario with sample size n=100 and theta=1,death rate=0.5##
    ##idx=3 for the scenario with sample size n=100 and theta=1,death rate=0.4 ; idx=4 for the scenario with sample size n=100 and theta=1.5,death rate=0.6##
    ##idx=5 for the scenario with sample size n=100 and theta=1.5 ,death rate=0.5; idx=6 for the scenario with sample size n=100 and theta=1.5,death rate=0.4##
    ##idx=7 for the scenario with sample size n=100 and theta=2 ,death rate=0.6; idx=8 for the scenario with sample size n=100 and theta=2,death rate=0.5##
    ##idx=9 for the scenario with sample size n=100 and theta=2,death rate=0.4; idx=10 for the scenario with sample size n=200 and theta=1,death rate=0.6##
    ##idx=11 for the scenario with sample size n=200 and theta=1 ,death rate=0.5; idx=12 for the scenario with sample size n=200 and theta=1,death rate=0.4##
    ##idx=13 for the scenario with sample size n=200 and theta=1.5 ,death rate=0.6; idx=14 for the scenario with sample size n=200 and theta=1.5,death rate=0.5##
    ##idx=15 for the scenario with sample size n=200 and theta=1.5 ,death rate=0.4; idx=16 for the scenario with sample size n=200 and theta=2,death rate=0.6##
    ##idx=17 for the scenario with sample size n=200 and theta=2 ,death rate=0.5; idx=18 for the scenario with sample size n=200 and theta=2,death rate=0.4##

    ##idx=19 for the scenario with sample size n=400 and theta=1,death rate=0.6##
    ##idx=20 for the scenario with sample size n=400 and theta=1 ,death rate=0.5; idx=21 for the scenario with sample size n=400 and theta=1,death rate=0.4 ##
    ##idx=22 for the scenario with sample size n=400 and theta=1.5 ,death rate=0.6 ; idx=23 for the scenario with sample size n=400 and theta=1.5,death rate=0.5 ##
    ##idx=24 for the scenario with sample size n=400 and theta=1.5 ,death rate=0.4; idx=25 for the scenario with sample size n=400 and theta=2,death rate=0.6 ##
    ##idx=26 for the scenario with sample size n=400 and theta=2 ,death rate=0.5 ; idx=27 for the scenario with sample size n=400 and theta=2,death rate=0.4 ##
  ...
  ######################################################################
  ######   generate new patient data using time-invarying model ##########
  ######################################################################
  ...
  b3=2             ###beta_R1=2#######
  b4=2             ###beta_R2=2#######
  ##### use time-invarying model fit time-varying prediction dataset #######
  ...
  phi=rnorm(n_predict)
  lam1=exp(b01+phi+x1*b1+x2*b2)
  lam2=exp(b02+phi+x1*b3+x2*b4)
  u=runif(n_patient)
  t1=-log(u)/lam1
  c1=runif(n_predict,0,UU)
  y1=pmin(t1,c1)
  quantile(y1)[2]
  s1=(t1<=c1)
  ...
  for(n_i in 1:n_predict){
    ...
    repeat{
      ...
      if(sum(t2)+r_new<=y1[n_i]&length(t2)<J+1) {
        t2=c(t2,r_new)
        t3=c(t3,sum(t2))
      }
      else if (sum(t2)+r_new>y1[n_i]&length(t2)<J+1) {
    ...
    s3=1-s2
    s3[length(s2)]=(s1[n_i])
    main_predict[[n_i]]=cbind(id=n_i,y2=t2,s1=s1[n_i],s2,s3,x1=x1[n_i],x2=x2[n_i],y1=y1[n_i],t3=t3)
  }


  main_predict=data.frame(do.call("rbind",main_predict))
  ...
  modJoint.gap<- frailtyPenal(Surv(y2,s2) ~ cluster(id) +
                                x1+x2+terminal(s3),formula.terminalEvent = ~ x1+x2,data = main_predict, n.knots = 8, kappa = c(2,3))

  phi_est_SF<-modJoint.gap$alpha
  y1<-vector()
  s3<-vector()
  prediction<-vector()
  for (i in unique(main_predict$id)){
    ni<-dim(main_predict[main_predict$id==i,])[1]
    y1[i]<-max(main_predict[main_predict$id==i,]$y1)
    s3[i]<-main_predict[main_predict$id==i,]$s3[ni]
  ...
  for(t_prime in t_s){
    ...
    for (t_final in t_f){
      ...
      for(k in 1:length(unique(main_predict$id))){

        main_predict_p=main_predict[main_predict$id==k,]
        ...
        prediction[k]<-exp(-t_final*exp(b1_est_SF*main_predict_p$x1[1]+b2_est_SF*main_predict_p$x2[1]+phi_est_SF))
      }



      s_score_terminal_SF=sbrier(Surv(y1,s3),prediction,btime =t_final)
  ...
  for(t_prime in t_s){
    ...
    for (t_final in t_f){
      ...
      for(k in 1:n_predict){
        ...
        main_predict_i=main_predict[main_predict$id==k,]
        if(nrow(main_predict_i)==1|main_predict_i$y1[1]<t_prime) next;
        s1_t2[k]=(t1[k]<c1[k])
        y1_t2[k]=min(t1[k],c1[k])
        s1_t[k]=(t1[k]<c1[k])
        y1_t[k]=min(t1[k],c1[k])
        main_predict_p=main_predict_i[main_predict_i$t3<t_prime,]
        main_predict_p$y1=t_prime
        main_predict_p$y2[nrow(main_predict_p)]=t_prime-main_predict_p$t3[nrow(main_predict_p)]
        main_predict_p$s2[nrow(main_predict_p)]=0

        main_predict_s=main_predict_i[nrow(main_predict_i),]
        ...
        for(m in slice){
          ...
          for(un_i in 2:10){
            ...
            if  (result$sigma[m]<-Inf){result$sigma[m]<-phiN_est_list$sigma[m]}
            phiN_sample[un_i]= metroplis_phi_id_dep(result$b1[m], result$b2[m], result$b3[m], result$b4[m], result$b01[m], result$b02[m],
                    result$sigma[m],result$mu[m],result$theta[m],main_predict_p$x1, main_predict_p$x2, main_predict_p$y1, main_predict_p$y2, main_predict_p$s3,main_predict_p$s2,
                    nrow(main_predict_p),phiN_sample[un_i-1],0.1);
        ...
        predict2[k]=exp(-t_final*exp(b1_est*main_predict_p$x1[1]+b2_est*main_predict_p$x2[1]+b01_est+phiN_est2))
        ...
        for(m in slice){
          ...
          #####MCMC begin######
          for(un_i in 2:10){
            phiN_sample[un_i]= metroplis_phi_id(phiN_est_list$b1[m], phiN_est_list$b2[m], phiN_est_list$b3[m], phiN_est_list$b4[m], phiN_est_list$b01[m], phiN_est_list$b02[m],
                    phiN_est_list$theta[m],phiN_est_list$sigma[m],main_predict_p$x1, main_predict_p$x2, main_predict_p$y1, main_predict_p$y2, main_predict_p$s3,main_predict_p$s2,
                    nrow(main_predict_p),phiN_sample[un_i-1],0.1);
        ...
        predict_terminal[k]=exp(-t_final*exp(b1_est1*main_predict_p$x1[1]+b2_est1*main_predict_p$x2[1]+b01_est1+phiN_est))
      }

...
double LL_dep_vec(NumericVector b1,NumericVector b2,double b01, double b02,NumericVector phi,
...
double LLi_dep_vec(NumericVector b1,NumericVector b2,double b01, double b02,NumericVector phi,
               double mu,NumericVector theta, NumericMatrix x,NumericVector y1,NumericVector y2,
...
double LL_id_dep_vec(NumericVector b1,NumericVector b2,double b01, double b02,double phi,
                 double mu,NumericVector theta, NumericMatrix x,NumericVector y1,NumericVector y2,
...
double target_phi_dep_vec(NumericVector b1, NumericVector b2,double b01, double b02,
                      ...
                      IntegerVector ni,IntegerVector maxni)
{
        ...
        sum=LLi_dep_vec(b1,b2,b01,b02,tem,mu,theta,x, y1,y2, s1, s2,id,N,p,j_start,j_end,ni,maxni)-0.5*func;
        return (sum);
}
...
double normalRandom_vec() {
        ...
        double v1=rand_gen_vec();
        double v2=rand_gen_vec();
...
double metroplis_phi_dep_vec(NumericVector b1,NumericVector b2,double b01, double b02,NumericVector phi_curr,double sigma, double mu,
                         ...
                         IntegerVector j_start,IntegerVector j_end,IntegerVector ni,IntegerVector maxni)
{
        ...
        double sd = 1.0;
        double mean = 0.0;
        double rnorm = normalRandom_vec()*sd+mean;
        x1_curr=x1_prev+scale*rnorm;
        temp1=target_phi_dep_vec(b1,b2,b01,b02,phi_curr,sigma,mu,theta,x,y1,y2,s1,s2,id,no_patient,N,x1_curr,j,p,j_start,j_end,ni,maxni);
        temp2=target_phi_dep_vec(b1,b2,b01,b02,phi_curr,sigma,mu,theta,x,y1,y2,s1,s2,id,no_patient,N,x1_prev,j,p,j_start,j_end,ni,maxni);
        lgratio=temp1-temp2;
...
double metroplis_phi_id_dep_vec(NumericVector b1,NumericVector b2,double b01, double b02,double sigma, double mu,
                            NumericVector theta,NumericMatrix x,NumericVector y1,NumericVector y2,IntegerVector s1,IntegerVector s2,
                            int N, double x1_prev,double scale,int p)
{
        ...
        double rnorm = normalRandom_vec()*sd+mean;
        x1_curr=x1_prev+scale*rnorm;
        temp1=LL_id_dep_vec(b1,b2,b01,b02,x1_curr,mu,theta,x,y1,y2,s1,s2,N,p)+R::dnorm(x1_curr,0,sigma,1);
        temp2=LL_id_dep_vec(b1,b2,b01,b02,x1_prev,mu,theta,x,y1,y2,s1,s2,N,p)+R::dnorm(x1_prev,0,sigma,1);
        lgratio=temp1-temp2;
...
double target_theta_dep_vec(NumericVector b1,NumericVector b2,double b01, double b02,
                        ...
                        int k,IntegerVector maxni,int p,IntegerVector j_start,IntegerVector j_end)
{
        ...
        sum=LLi_dep_vec(b1,b2,b01,b02,phi,mu,temp,x, y1,y2, s1, s2, id,N,p,j_start,j_end,ni,maxni)-0.5*func;
        return (sum);
}
...
double metroplis_theta_dep_vec(NumericVector b1,NumericVector b2,double b01, double b02,
...

{
        ...
        temp1=target_theta_dep_vec(b1,b2,b01,b02,phi,mu,theta_curr, rho, sigma_epsilon, x,y1,y2,s1,s2,id,N,ni, x1_curr,j,maxni,p,j_start,j_end);
        temp2=target_theta_dep_vec(b1,b2,b01,b02,phi,mu,theta_curr,rho, sigma_epsilon, x,y1,y2,s1,s2,id,N,ni, x1_prev,j,maxni,p,j_start,j_end);
        lgratio=temp1-temp2;
...
List MH_dep_vec(NumericMatrix b1,NumericMatrix b2,NumericVector b01,
        ...
        for (i = 1; i < n_sample; i++)
        {
                ...
                temp1=LLi_dep_vec(y_new1,b2.row(i-1),b01[i-1],b02[i-1],phi,mu[i-1],theta,x, y1,y2, s1, s2,id,N_p,p,j_start,j_end,ni,maxni);
                temp2=LLi_dep_vec(b1.row(i-1),b2.row(i-1),b01[i-1],b02[i-1],phi,mu[i-1],theta,x, y1,y2, s1, s2,id,N_p,p,j_start,j_end,ni,maxni);
                lgratio=temp1-temp2;
                ...
                temp1=LLi_dep_vec(b1.row(i),y_new1,b01[i-1],b02[i-1],phi,mu[i-1],theta,x, y1,y2, s1, s2,id,N_p,p,j_start,j_end,ni,maxni);
                temp2=LLi_dep_vec(b1.row(i),b2.row(i-1),b01[i-1],b02[i-1],phi,mu[i-1],theta,x, y1,y2, s1, s2,id,N_p,p,j_start,j_end,ni,maxni);
                lgratio=temp1-temp2;
                ...
                temp1=LLi_dep_vec(b1.row(i),b2.row(i),y_new2,b02[i-1],phi,mu[i-1],theta,x, y1,y2, s1, s2,id,N_p,p,j_start,j_end,ni,maxni);
                temp2=LLi_dep_vec(b1.row(i),b2.row(i),b01[i-1],b02[i-1],phi,mu[i-1],theta,x, y1,y2, s1, s2,id,N_p,p,j_start,j_end,ni,maxni);
                lgratio=temp1-temp2;
                if (lgratio>=log(u))  b01[i]=y_new2 ;
                else b01[i]=b01[i-1];

                y_new2=b02[i-1]+scale[5]*R::rnorm(0,1);
                temp1=LLi_dep_vec(b1.row(i),b2.row(i),b01[i],y_new2,phi,mu[i-1],theta,x, y1,y2, s1, s2,id,N_p,p,j_start,j_end,ni,maxni);
                temp2=LLi_dep_vec(b1.row(i),b2.row(i),b01[i],b02[i-1],phi,mu[i-1],theta,x, y1,y2, s1, s2,id,N_p,p,j_start,j_end,ni,maxni);
                lgratio=temp1-temp2;
                ...
                if(y_new2>0){
                        temp1=LL_dep_vec(b1.row(i),b2.row(i),b01[i],b02[i],phi,y_new2,theta,x, y1,y2, s1, s2,id,N_p,p,ni,maxni)+R::dgamma(y_new2,0.001,1000,1);
                        temp2=LL_dep_vec(b1.row(i),b2.row(i),b01[i],b02[i],phi,mu[i-1],theta,x, y1,y2, s1, s2,id,N_p,p,ni,maxni)+R::dgamma(mu[i-1],0.001,1000,1);
                        lgratio=temp1-temp2;
                ...
                for (j = 0; j < no_pat_p; j++)
                {
                        ...
                        printf("%d\n",j);
                        x1_prev=phi[j];
                        temp=metroplis_phi_dep_vec(b1.row(i),b2.row(i),b01[i],b02[i],phi,sigma[i-1],mu[i],theta,x,y1,y2,s1,s2,id,no_pat_p,N_p,x1_prev,j,scale[7],p,j_start,j_end,ni,maxni);
                        phi[j]=temp;
                        printf("%f\n",phi[j]);
                        phi=center_dep_vec(phi,no_pat_p);
                }



                sig_pat=target_sigma_dep_vec(phi,no_pat_p);
                printf("%f\n",sig_pat);
                ...
                for (j = 0; j < N_p; j++)
                {
                        ...
                        printf("%d\n",j);
                        x1_prev=theta[j];
                        temp=metroplis_theta_dep_vec(b1.row(i),b2.row(i),b01[i],b02[i],phi,mu[i],theta,rho[i-1],sigma_epsilon[i-1],x,y1,y2,s1,s2,id,N_p,ni,x1_prev,j,scale[8],maxni,p,j_start,j_end);
                        theta[j]=temp;
                        printf("%f\n",theta[j]);
                        theta=center_dep_vec(theta,N_p);
                }
                ...
                theta_pow=theta_pow_sum_dep_vec(theta,N_p,ni);
                theta_lag=theta_lag_sum_dep_vec(theta,N_p,ni);
                ...
                sig_theta=target_sigma_epsilon_dep_vec(theta,N_p,ni,rho[i]);
